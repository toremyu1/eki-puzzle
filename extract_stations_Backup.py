...
def get_todays_sub_pages():
    ...
    if not chunks[prev_weekday]:
        chunks[prev_weekday] = ["あ"]

    # ★テスト用に「あ」のページだけを強制的に指定します
    # return ["ち-て"], prev_weekday

    return chunks[prev_weekday], prev_weekday
...
def extract_and_count_stations():
    ...
    SUB_PAGES, pre_weekday_num = get_todays_sub_pages()
    weekdays_str = ["月", "火", "水", "木", "金", "土", "日"]
    print(f"本日は【{weekdays_str[pre_weekday_num + 1]}曜日】の1日前の【{weekdays_str[pre_weekday_num]}曜日】の割り当て分（{'、'.join(SUB_PAGES)}）を抽出します。")
